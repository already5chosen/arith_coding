...
void arithmetic_decode_model_t::prepare(const uint16_t c2range[256])
{
  ...
  m_ci2lo[ci] = VAL_RANGE;
  for (unsigned i = ci+1; i < 258; ++i)
    m_ci2lo[i] = VAL_RANGE+i;
...
int arithmetic_decode_model_t::decode(uint8_t* dst, int dstlen, const uint8_t* src, int srclen)
{
  ...
  for (int i = 0; ; ) {
    ...
    unsigned ci = val2ci(val_h*invRange); // can be off by +1

    // if (i >= 0*4600 && i < 4650) {
      // double ra = range * (1.0/VAL_RANGE);
      // int64_t cLo = m_ci2lo[c+0];
      ...
      // , i, ci, val_h, val_l, range
      // , m_ci2lo[c+0], m_ci2lo[ci+1]
      // , val_h*invRange
