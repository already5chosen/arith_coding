...
{
  bn192_copy(st_pub_key.X, key[0]);
  bn192_copy(st_pub_key.Y, key[1]);
...
// derived from ossl_ecdsa_verify_sig()
// in \openssl-1.1.1\crypto\ec\ecdsa_ossl.c
// return:
//        1 = valid signature
//        0 = invalid signature
//       -1 = internal error
int secp192k1_ecdsa_verify(const bn_t digest, const bn_t signature[2])
{
  ...
  const bn_word_t* sig_r = signature[0];
  const bn_word_t* sig_s = signature[1];
