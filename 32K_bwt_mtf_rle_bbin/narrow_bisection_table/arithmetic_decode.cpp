...
static int decode(
  ...
  const uint8_t* src,
  int            srclen,
  const uint8_t  bisectionTab[256])
{
  uint8_t  cntrs[258] = {0};
  uint8_t  prevQh[258];
  uint16_t currH[258];
  ...
  while (dstlen > 0) {
    ...
    int tLo = 0, tHi = 256, tIdx = 0;
    do {
      unsigned idx = tabOffset + tIdx;
      ...
      int tMid = bisectionTab[tIdx];
      int dIdx = b ? tMid - tLo : 0;
      tLo = b ? tMid + 1 : tLo;
      tHi = b ? tHi : tMid;
      tIdx += 1 + dIdx;
    } while (tLo != tHi);
...
static void BuildBisectionTableRecursive(uint8_t* dst, unsigned lo, unsigned hi, unsigned rat)
{
  if (lo != hi) {
    unsigned mid = (lo*rat+hi*(32-rat))/32;
    dst[0] = mid;
    BuildBisectionTableRecursive(dst+1, lo, mid, rat);
    BuildBisectionTableRecursive(dst+1+mid-lo, mid+1, hi, rat);
  }
}

static void BuildBisectionTable(uint8_t dst[256], unsigned midPt0, unsigned midFactor0, unsigned midFactor1)
{
  dst[0] = 1;      // [0..256]
  dst[1] = 0;      // [0..1]
  dst[2] = midPt0; // [2..256]
  BuildBisectionTableRecursive(&dst[2+1],          2, midPt0, midFactor0);
  BuildBisectionTableRecursive(&dst[2+1+midPt0-2], midPt0+1, 256, midFactor1);
}
  ...
  int*           pInfo)
{
  ...
  uint8_t bisectionTab[256];
  BuildBisectionTable(bisectionTab, mid0+1, midFactor0, midFactor1);
